...
- ``meta``: provenance. ``source`` is ``"fixtures"`` or ``"live"`` and drives
  the honesty footer in reports.
"""
...
class Store:
    ...
    def upsert_snapshot(self, snap: BookSnapshot, source: str = "live") -> None:
        ...
        self.conn.execute(
            ...
            (
                ...
                json.dumps(snap.to_dict()), source,
            ),
        )
        self.conn.commit()
    ...
    def snapshots_for_date(self, date: str) -> list[sqlite3.Row]:
        return self.conn.execute(
            "SELECT * FROM snapshots WHERE ts LIKE ? ORDER BY ts, venue",
            (f"{date}%",),
        ).fetchall()

    def dislocations_for_date(self, date: str) -> list[sqlite3.Row]:
    ...
    def snapshot_counts_by_venue(self, date: str) -> list[sqlite3.Row]:
        return self.conn.execute(
